...
class WIDGET_footerMenuLoginTmpl {
	...
	public function wFooter($fn_args)
	{
		...
		if($fn_q_menus)
		{
			...
			if($fn_q_structures)
			{
				...
				foreach($fn_menu_tree as $mk => $mv)
				{
					...
					self::$fn_xtemplate_parse['parse'][] = "{$fn_args['stage_id']}.footerMenuLoginTmpl.row_item";
					//anti loop
		...
		self::$fn_xtemplate_parse['parse'][] = "{$fn_args['stage_id']}.footerMenuLoginTmpl";
