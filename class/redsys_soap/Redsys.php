...
class Redsys
{
     ...
     * 
     * @access public
     * @param string $lang (default: '2') español
     * @return void
     */
    public function setLang($lang = 'es')
    {
		...
		$this->_parameters['DS_MERCHANT_CONSUMERLANGUAGE'] = $lang_consumer;
    }
    ...
    public function firePayment($type = 'test')
    {
        ...
        $xml = $this->buildXML();
        $client = new \SoapClient($this->getEnviroment($type), array('cache_wsdl' => WSDL_CACHE_NONE));
    ...
    public function createForm($type = 'test', $options = [])
    {
        ...
        $form = '<form action="'.$environment.'" method="post" name="'.$options['form_name'].'"><input type="hidden" name="Ds_MerchantParameters" value="'.$this->generateMerchantParameters().'"/><input type="hidden" name="Ds_Signature" value="'.$this->generateSignature($this->generateMerchantParameters(), $this->getOrder()).'"/><input type="hidden" name="Ds_SignatureVersion" value="HMAC_SHA256_V1"/><input type="submit" name="submitname" value="'.$options['submit_value'].'"></form>';
